...
int cMsgAddSenderToHistory(void *vmsg, char *name) {
    ...
    if (!exists) {
        ...
        index = 0;
        if (item->count >= msg->historyLengthMax) {
            len = msg->historyLengthMax - 1;
            index = item->count - len;
        }
...
int cMsgPayloadCopy(const void *vmsgFrom, void *vmsgTo) {
  ...
  while (item != NULL) {
    ...
    if ((strcmp(item->name, "cMsgSenderHistory") == 0) &&
 ...
 * payload in a form for sending over the network. It stores that string
...
int cMsgPayloadUpdateText(const void *vmsg) {
  ...
  /* add each item's length to total */
  item = msg->payload;
  while (item != NULL) {
    totalLen += item->length;
    item = item->next;
  }
  totalLen += numDigits(msg->payloadCount, 0) + 1; /* send count & newline first */
...
void cMsgPayloadPrintout(const void *msg, int level) {
  ...
  for (k=0; k<namesLen; k++) {
    ...
    switch (types[k]) {
      ...
      case CMSG_CP_DBL:
        {double d;   ok=cMsgGetDouble(msg, name, &d); if(ok==CMSG_OK) printf(" (double): %.16lg\n", d);} break;
      case CMSG_CP_FLT:
...
/**
 * This routine returns the value of the given field as a binary array if it exists.
...
int cMsgGetBinary(const void *vmsg, const char *name, char **val, int *len, int *endian) {
  ...
  if (msg == NULL || name == NULL ||
      val == NULL || len == NULL  || endian == NULL) return(CMSG_BAD_ARGUMENT);
  ...
  if (item->type != CMSG_CP_BIN || item->count < 1 || item->array == NULL) {
    releaseMutex();
    return(CMSG_BAD_FORMAT);
  }
